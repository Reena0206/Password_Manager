...
password_entry = Entry(root,width=87)
...
def src_btn():
    ...
    for i,j in d.items():
        if i==web:
            # print(f"{i}:{j}")
            # for d in j:
            # web_entry.insert(0,i)
            # em_entry.delete(0,END)
            # em_entry.insert(0,j['email'])
            # password_entry.insert(0,j['password'])
            messagebox.showinfo(i,f"Email: {'email'}\nPassword: {j['password']}")

            break
    else:
        msg = messagebox.showerror('Search Data',"Data Not Found")
...
search_btn = Button(root,text="Search",width=20,font=('Arial',10,'bold'),command=src_btn)
search_btn.grid(row=1,column=2,sticky='w')
gen_btn = Button(root,text="Generate Password",width=20,font=('Arial',10,'bold'),command=gen_pswd)
